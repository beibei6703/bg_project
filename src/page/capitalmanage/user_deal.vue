...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getdetail(val) {
        this.$router.push({path: '/tradedetail', query: {id: val.id, tab:1,}});
      },
      exports(){
        if(this.serial_number!=''||this.time!='' ||this.type!=''){
          window.location.href = this.deal_dayfile+'?tab=1&serial_number='+this.serial_number+'&time='+this.time+'&type='+this.type+'&token='+localStorage.getItem("token")
        }else{
          this.$message({
            type: 'error',
            message: '请输入筛选条件'
